...
import {
  ...
  ThumbsUp, ThumbsDown, AlertOctagon, User, Calendar, ArrowRight, ArrowLeft,
} from 'lucide-react';
...
function analyzeScreenshot(fileName: string, fileSize: number): ScanResult {
  ...
  if (directMatched) {
    indicators.push({ label: `Filename contains scam indicator: "${directMatched}" (typo/different spelling also detected)`, found: true, type: 'negative' });
    score += 55;
...
export default function ScamDetector() {
  ...
  const [reportFormData, setReportFormData] = useState({
    fraudType: '', name: '', mobile: '', email: '', amountLost: '', dateOfFraud: '',
    description: '', fraudsterMobile: '', fraudsterUpi: '', fraudsterLink: '',
    evidence: [] as File[],
  });
  const [reportSubmitting, setReportSubmitting] = useState(false);
  const [complaintGenerated, setComplaintGenerated] = useState('');
  const [reportRiskScore, setReportRiskScore] = useState(0);

  const calculateReportRiskScore = () => {
    let score = 0;
    if (reportFormData.amountLost && parseInt(reportFormData.amountLost) > 10000) score += 30;
    else if (reportFormData.amountLost && parseInt(reportFormData.amountLost) > 5000) score += 20;
    else if (reportFormData.amountLost && parseInt(reportFormData.amountLost) > 1000) score += 10;
    if (reportFormData.fraudType) score += 15;
    if (reportFormData.fraudsterMobile) score += 15;
    if (reportFormData.fraudsterUpi) score += 10;
    if (reportFormData.fraudsterLink) score += 10;
    if (reportFormData.evidence.length > 0) score += 10;
    if (reportFormData.description.length > 20) score += 10;
    return Math.min(score, 100);
  };

  const generateReportComplaint = () => {
    const fraudLabel = reportFraudTypes.find(f => f.id === reportFormData.fraudType)?.label || 'Cyber Fraud';
...
Complainant Details:
- Name: ${reportFormData.name || 'N/A'}
- Mobile: ${reportFormData.mobile || 'N/A'}
- Email: ${reportFormData.email || 'N/A'}

Incident Details:
Nature of Fraud: ${fraudLabel}
Date of Incident: ${reportFormData.dateOfFraud || 'N/A'}
Amount Involved: Rs. ${reportFormData.amountLost || '0'}

Description of Incident:
${reportFormData.description || 'No details provided'}

Fraudster Details:
- Mobile Number: ${reportFormData.fraudsterMobile || 'N/A'}
- UPI ID: ${reportFormData.fraudsterUpi || 'N/A'}
- Suspicious Link: ${reportFormData.fraudsterLink || 'N/A'}

Evidence Attached:
${reportFormData.evidence.length > 0 ? reportFormData.evidence.map(e => `- ${e.name}`).join('\n') : 'No evidence attached'}
...
3. Visit: Nearest Police Station with Cyber Crime Cell`;
    ...
    setReportRiskScore(calculateReportRiskScore());
  };

  const handleReportSubmit = () => {
    setReportSubmitting(true);
    setTimeout(() => {
      generateReportComplaint();
      setReportStep(4);
  ...
  const handleReportFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setReportFormData(prev => ({ ...prev, evidence: [...prev.evidence, ...files] }));
  };

  const removeReportFile = (index: number) => {
    setReportFormData(prev => ({ ...prev, evidence: prev.evidence.filter((_, i) => i !== index) }));
  };
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-8 animate-fade-in-up">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">
            Check any suspicious message, link, QR code, UPI ID, phone number, or email.
            AI will analyze in real-time and tell you whether it is a scam or safe.
          </p>
        </div>
            ...
            {result.status === 'fraud' && (
              <div className="mt-6 p-4 rounded-xl bg-red-500/10 border border-red-500/30 animate-scale-in">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <button onClick={() => { setActiveTab('report'); setScamText(''); setResult(null); }} className="flex items-center gap-3 p-3 bg-red-500/20 rounded-lg hover:bg-red-500/30 transition-colors">
                    <FileText className="w-5 h-5 text-red-400" />
                    <div className="text-left">
                      <p className="text-sm text-white font-medium">File Complaint</p>
                      <p className="text-xs text-gray-400">AI-generated complaint</p>
                    </div>
                  </button>
                </div>
              </div>
              ...
              {[1, 2, 3, 4].map((s) => (
                <div key={s} className="flex items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-medium transition-all ${
                    reportStep >= s ? 'bg-primary-600 text-white' : 'bg-dark-700 text-gray-400'
                  }`}>
                    {s}
                  </div>
                  {s < 4 && (
                    <div className={`w-12 h-1 mx-1 rounded transition-all ${reportStep > s ? 'bg-primary-600' : 'bg-dark-700'}`} />
                  )}
                </div>
              ))}
            </div>
          ...
          {reportStep === 1 && (
            <div>
              ...
              <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                {reportFraudTypes.map((type) => (
                  <button key={type.id} onClick={() => setReportFormData(prev => ({ ...prev, fraudType: type.id }))}
                    className={`p-4 rounded-xl text-left transition-all ${reportFormData.fraudType === type.id ? 'bg-primary-600/20 border-2 border-primary-500' : 'bg-dark-700 border-2 border-transparent hover:border-dark-600'}`}>
              ...
              <div className="mt-6 flex justify-end">
                <button onClick={() => setReportStep(2)} disabled={!reportFormData.fraudType}
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={reportFormData.name} onChange={(e) => setReportFormData(prev => ({ ...prev, name: e.target.value }))}
                ...
                <div>
                  ...
                  <div className="relative">
                    <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                    <input type="tel" value={reportFormData.mobile} onChange={(e) => setReportFormData(prev => ({ ...prev, mobile: e.target.value }))}
                ...
                <div>
                  ...
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                    <input type="email" value={reportFormData.email} onChange={(e) => setReportFormData(prev => ({ ...prev, email: e.target.value }))}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" value={reportFormData.amountLost} onChange={(e) => setReportFormData(prev => ({ ...prev, amountLost: e.target.value }))}
                      placeholder="Amount lost" className="input-field pl-10" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="date" value={reportFormData.dateOfFraud} onChange={(e) => setReportFormData(prev => ({ ...prev, dateOfFraud: e.target.value }))}
                      className="input-field pl-10" />
                  </div>
                </div>
              </div>
              <div className="flex justify-between mt-6">
                <button onClick={() => setReportStep(1)} className="btn-secondary flex items-center gap-2">
                  <ArrowLeft className="w-4 h-4" /> Back
                </button>
                <button onClick={() => setReportStep(3)} disabled={!reportFormData.name || !reportFormData.mobile}
                  className="btn-primary flex items-center gap-2 disabled:opacity-50">
                  Next <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            ...
            <div>
              ...
              <div className="space-y-4 mb-6">
                <div>
                  ...
                  <textarea value={reportFormData.description} onChange={(e) => setReportFormData(prev => ({ ...prev, description: e.target.value }))}
                    placeholder="Describe the fraud incident in detail..." rows={4} className="input-field resize-none" />
                </div>
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block mb-2 text-sm text-gray-300">Fraudster's Mobile (if known)</label>
                    <input type="text" value={reportFormData.fraudsterMobile} onChange={(e) => setReportFormData(prev => ({ ...prev, fraudsterMobile: e.target.value }))}
                      placeholder="Fraudster's mobile number" className="input-field" />
                  </div>
                  <div>
                    <label className="block mb-2 text-sm text-gray-300">Fraudster's UPI ID (if known)</label>
                    <input type="text" value={reportFormData.fraudsterUpi} onChange={(e) => setReportFormData(prev => ({ ...prev, fraudsterUpi: e.target.value }))}
                      placeholder="example@paytm" className="input-field" />
                  </div>
                </div>
                <div>
                  <label className="block mb-2 text-sm text-gray-300">Suspicious Link (if any)</label>
                  <input type="text" value={reportFormData.fraudsterLink} onChange={(e) => setReportFormData(prev => ({ ...prev, fraudsterLink: e.target.value }))}
                ...
                <div>
                  ...
                  <label className="block">
                    <div className="border-2 border-dashed border-dark-600 rounded-xl p-6 text-center cursor-pointer hover:border-primary-500 transition-colors">
                      <input type="file" multiple accept="image/*,.pdf" onChange={handleReportFileUpload} className="hidden" />
                ...
                {reportFormData.evidence.length > 0 && (
                  <div className="space-y-2">
                    {reportFormData.evidence.map((file, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-dark-700 rounded-lg">
                        <div className="flex items-center gap-2">
                          <FileText className="w-4 h-4 text-primary-400" />
                          <span className="text-sm text-gray-300">{file.name}</span>
                        </div>
                        <button onClick={() => removeReportFile(i)} className="text-red-400 hover:text-red-300 text-sm">Remove</button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="flex justify-between">
                <button onClick={() => setReportStep(2)} className="btn-secondary flex items-center gap-2">
                  <ArrowLeft className="w-4 h-4" /> Back
                </button>
                <button onClick={handleReportSubmit} disabled={reportSubmitting}
                  ...
                  {reportSubmitting ? (
                    ...
                    <><Shield className="w-5 h-5" /><span>Generate Complaint</span></>
                  )}
                </button>
              </div>
            </div>
          ...
          {reportStep === 4 && (
            <div>
              <div className="text-center mb-6">
                ...
                <p className="text-gray-400 mt-2">AI has prepared your complaint</p>
              </div>
              <div className="mb-6 p-4 rounded-xl bg-dark-700">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-gray-400">Risk Score</span>
                  <span className={`text-lg font-bold ${reportRiskScore >= 70 ? 'text-red-400' : reportRiskScore >= 40 ? 'text-amber-400' : 'text-green-400'}`}>
                    {reportRiskScore}/100
                  </span>
                </div>
                <div className="h-2 bg-dark-600 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full ${reportRiskScore >= 70 ? 'bg-red-500' : reportRiskScore >= 40 ? 'bg-amber-500' : 'bg-green-500'}`}
                    style={{ width: `${reportRiskScore}%` }} />
                </div>
              </div>
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
              ...
              <div className="grid sm:grid-cols-3 gap-3 mb-6">
                ...
                <button onClick={() => {
                  setReportFormData({ fraudType: '', name: '', mobile: '', email: '', amountLost: '', dateOfFraud: '', description: '', fraudsterMobile: '', fraudsterUpi: '', fraudsterLink: '', evidence: [] });
                  setReportStep(1); setComplaintGenerated('');
